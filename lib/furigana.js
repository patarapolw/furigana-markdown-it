...
/** @typedef {{ body: string; top?: string }} Rubified */
...
function process(state, silent, options) {
  const ruby = rubyHelper.parse(state, options);
  if (ruby === null) {
    return false;
  }
  ...
  if (ruby.toptext === null && options.lang) {
    state.push("rspan_open", "span", 1).attrSet("lang", options.lang);
    state.push("text", "", 0).content = ruby.body;
    state.push("rspan_close", "span", -1);
    return true;
  }
  ...
  if (emphasisDotsIndicatorRegex.test(ruby.toptext)) {
    content = rubifyEveryCharacter(ruby.body, ruby.toptext);
    exclude.fallbackParens = true;
  } else {
    content = matchFurigana(ruby.body, ruby.toptext, options);
  }

  rubyHelper.addTag(state, content, options, exclude);
