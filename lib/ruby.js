...
/**
 * Parses the [body]{top} syntax and returns
 ...
 * @returns {{ ruby: import('./furigana').Rubified, nextPos: int }}
 *
 ...
 * eventually resulting in consecutive `<ruby>` tags.
 *
 ...
 *
 * @example
 * addTag(state, ['猫', 'ねこ', 'と', '', '犬', 'いぬ'], options)
...
function addTag(state, content, options, exclude = {}) {
  ...
  for (let i = 0; i < content.length; i += 2) {
    ...
    /**
     * Ruby tags should be without empty `<rt></rt>`, according to whatwg.
     * This will also allow a column of ruby tags / nested ruby tags.
     *
    ...
    if (!exclude.fallbackParens) {
      ...
      state.push("text", "", 0).content = options.fallbackParens.charAt(0);
    ...
    if (!exclude.fallbackParens) {
      state.push("rp_open", "rp", 1);
      state.push("text", "", 0).content = options.fallbackParens.charAt(1);
      state.push("rp_close", "rp", -1);
    }
